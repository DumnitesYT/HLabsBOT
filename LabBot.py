...
TEXTS = {
    "ru": {
        ...
        "theme_sent": "✅ Lazurite Dream 💙 загружена!\n\n{guide}",
        "btn_themes": "📁 Темы",
    ...
    "en": {
        ...
        "guide": "📱 *How to install*\n\n1️⃣ Download .hwt\n2️⃣ Copy to Huawei/Themes\n3️⃣ Close Themes app\n4️⃣ Open Themes → My themes\n\n✅ Done!",
...
def add_watermark_center(image_bytes, user_id, opacity=9):
    """Водяной знак со смещением вправо, очень слабый (opacity=9)"""
    ...
    while y < img.height:
        ...
        y += text_h + 30
    # Смещение вправо на 20 пикселей от центра
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with zipfile.ZipFile(ICONS_ZIP, 'r') as zicons:
                ...
                for i, name in enumerate(all_icons, 1):
                    ...
                    # Обновляем сообщение с именем текущей иконки (каждые 2 иконки для плавности)
                    if i % 2 == 0 or i == total:
            ...
            # Создаём файл icons (архив без сжатия)
            ...
            # description.xml по твоему шаблону
...
def send_lazurite_theme(chat_id, lang, msg_id=None):
    ...
    with open(THEME_FILE, 'rb') as f:
        bot.send_document(chat_id, f, caption=TEXTS[lang]["theme_sent"].format(guide=TEXTS[lang]["guide"]), reply_markup=after_download_menu(lang), parse_mode='Markdown')
...
def start(m):
    ...
    try:
        if len(m.text.split())>1 and m.text.split()[1]=='download_lazurite':
            send_lazurite_theme(m.chat.id, lang)
            return
    except: pass
...
def cb_handler(c):
    ...
    try:
        if c.data == 'themes':
            ...
            send_lazurite_theme(c.message.chat.id, lang, c.message.message_id)
        elif c.data == 'support':
...
if __name__ == '__main__':
    print("🤖 HuwiLabs bot — make your choose.")
